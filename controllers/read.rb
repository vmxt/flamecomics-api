...
class ReadController
  def self.fetch_read(series_id, chapter_id)
    ...
    series_title = doc.at_css('p[class*="TopChapterNavbar_series_title"]')&.text&.strip || 'Unknown Series'

    chapter_node = doc.at_css('p[class*="TopChapterNavbar_chapter_title"]')
    chapter_title = if chapter_node
                      chapter_node.text.gsub(/\s+/, ' ').strip
                    else
                      'Unknown Chapter'
                    end
