...
class BrowseController
  def self.fetch_series(query_string = "")
    ...
    doc.css('.mantine-Stack-root').each do |el|
      ...
      genres = el.css('.DescSeriesCard_categories__0736e .mantine-Badge-label').map(&:text).map(&:strip)

      description = el.at_css('.DescSeriesCard_description__XNkvv p')&.text&.strip || 'No Description'
