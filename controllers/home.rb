  ...
  def fetch_data
    begin
      ...
      document.css('.mantine-Carousel-slide').each do |elem|
        ...
        spotlight << { id: id, title: title, img_url: img_url, genres: genres }
      end
      ...
      if (staff_section = document.at_css('#staff-picks'))
        staff_section.css('.mantine-Grid-col').each do |elem|
          link = elem.at_css('a[href^="/series/"]')
          next unless link
      ...
      document.css('.m_96bdd299.mantine-Grid-col').each do |elem|
        ...
        chapters = elem.css("a[href*=\"/series/#{id}/\"]").map do |chapter_link|
          ...
          { chapter_id: chapter_id, chapter_title: chapter_title, chapter_date: chapter_date }
        end.compact
